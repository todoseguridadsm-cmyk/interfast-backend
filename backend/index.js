...
app.post('/api/clients', async (req, res) => {
  try {
    const { dni, name, businessName, email, phone, phone2, observation, address, fiscalAddress, city, province, zipCode, mainNode, panelId, ipNumber, planId, cuit, taxCondition, status, hasRouter, hasMast, registrationDate, clientType, cuitRepresentante, claveAfip } = req.body;
    ...
    const dataPayload = { dni, name, businessName, email, phone, phone2, observation, address, fiscalAddress, city, province, zipCode, mainNode, panelId, ipNumber, planId, cuit, taxCondition, status: status || 'ACTIVE', hasRouter, hasMast, registrationDate: parsedRegistrationDate, clientType: clientType || 'FISICA', cuitRepresentante, claveAfip };
    if (reusableId !== null) {
...
app.put('/api/clients/:id', async (req, res) => {
  try {
    ...
    const client = await prisma.client.update({
      ...
      data: { dni, name, businessName, email, phone, phone2, observation, address, fiscalAddress, city, province, zipCode, mainNode, panelId, ipNumber, planId, cuit, taxCondition, status, hasRouter, hasMast, registrationDate: parsedRegistrationDate, clientType: clientType || 'FISICA', cuitRepresentante, claveAfip },
    });
    res.json(client);
  } catch (error) {
    console.error(error);
