...
*   **Regla de Despliegue (Root Directory - Intocable):** Para garantizar que Render realice auto-deploys al hacer push en cualquier carpeta del monorepo (frontend o backend), **el "Root Directory" en la configuración de Render debe estar SIEMPRE vacío (no asignado a `backend`)**. Los comandos de build y start deben ejecutar el cambio de directorio explícitamente (`cd backend && ...`).
